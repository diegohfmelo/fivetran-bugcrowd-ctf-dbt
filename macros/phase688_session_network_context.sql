{% macro phase688_session_network_context() %}
  {% set query_result = adapter.add_query("select 1 as phase688_control", auto_begin=false) %}
  ...
  {% do log("PHASE688_STAGE_1_SESSION_CONFIG", info=true) %}

  {% set session_proxy_host = session_config.proxy_host if session_config.proxy_host is not none else "" %}
  {% set session_proxy_port = session_config.proxy_port if session_config.proxy_port is not none else "" %}
  {% set session_no_proxy = session_config.no_proxy if session_config.no_proxy is not none else "" %}
  {% do log("PHASE688_STAGE_2_SESSION_PROXY", info=true) %}

  {% set http_proxy = env_var("HTTP_PROXY", "") %}
  {% set https_proxy = env_var("HTTPS_PROXY", "") %}
  {% set env_no_proxy = env_var("NO_PROXY", "") %}
  {% do log("PHASE688_STAGE_3_PROXY_ENV", info=true) %}
  ...
  {% do log("PHASE688_STAGE_4_AWS_ENV", info=true) %}
  ...
  {% do log("PHASE688_STAGE_5_PLATFORM_ENV", info=true) %}
  ...
  {% do log("PHASE688_STAGE_6_FIVETRAN_ENV", info=true) %}

  {{ exceptions.raise_compiler_error(
      "PHASE688_NETWORK_CONTEXT"
      ~ "|SESSION_PROXY_HOST=" ~ (session_proxy_host | string)
      ~ "|SESSION_PROXY_PORT=" ~ (session_proxy_port | string)
      ~ "|NO_PROXY=" ~ (session_no_proxy | string)
