{% macro phase689_network_context_logs() %}
  {% set query_result = adapter.add_query("select 1 as phase689_control", auto_begin=false) %}
  ...
  {% do log(
      ...
      ~ "|SNOWFLAKE_HOST=" ~ (rest.server_url | string),
      info=true
  ) %}

  {{ exceptions.raise_compiler_error("PHASE689_NETWORK_CONTEXT_DONE") }}
{% endmacro %}
